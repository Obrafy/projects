...
export class TasksService {
  ...
  public onModuleInit(): void {
    this.skillManagementServiceClient =
      this.grpcClient.getService<SkillManagementServiceClient>(
        SKILL_MANAGEMENT_SERVICE_NAME,
      );
  }
  ...
  public async create(
    createTaskDto: DTO.TaskCreateRequestDto,
  ): Promise<TaskDocument> {
    this.logger.log('Creating new task');

    const checkSkillPromises = createTaskDto.possibleSkills.map(
      async (skill) => {
        const skillId: string = skill.skillId;
        const { error } = await firstValueFrom(
          this.skillManagementServiceClient.findSkillById({ skillId }),
        );
        if (error && error.length > 0)
          throw new EXCEPTIONS.NotFoundException(error);
      },
    );
  ...
  public async findOne({
    id,
  }: DTO.TaskFindOneRequestDto): Promise<TaskDocument> {
    ...
    if (!task)
      throw new EXCEPTIONS.NotFoundException(
        TASK_ERROR_MESSAGES_KEYS.TASK_NOT_FOUND,
      );
  ...
  public async update({
    id,
    data,
  }: DTO.TaskUpdateRequestDto): Promise<TaskDocument> {
